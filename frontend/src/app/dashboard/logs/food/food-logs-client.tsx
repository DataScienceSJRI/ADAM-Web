...
export type FoodLogParticipantSummary = ParticipantSummary;
...
export type FoodLogParticipantData = ParticipantData;
...
export function FoodLogsClient({
  initialParticipants,
  initialIsAdmin,
  initialSelectedId,
  ...
  initialParticipantData: ParticipantData | null;
  initialReviewsMap: Record<string, MealImageReview>;
  initialError?: boolean;
}) {
  const [participants, setParticipants] = useState<ParticipantSummary[]>(initialParticipants);
  const [isAdmin] = useState(initialIsAdmin);
  const [cohortFilter, setCohortFilter] = useState<CohortFilter>("actual");
  const [selectedId, setSelectedId] = useState<string | null>(initialSelectedId);
  const [participantDataById, setParticipantDataById] = useState<Record<string, ParticipantData>>(() => {
    if (!initialSelectedId || !initialParticipantData) return {};
  ...
  useEffect(() => {
    ...
    if (participantDataById[selectedId]) return;
  ...
  }, [participantDataById, selectedId]);

  const participantData = selectedId ? participantDataById[selectedId] ?? null : null;
  const detailLoading = loadingData || Boolean(selectedId && !participantData);
  ...
  useEffect(() => {
    if (Object.keys(initialReviewsMap).length > 0) return;
  ...
  }, [selectedId, visibleParticipants]);


  const handleDeleted = useCallback((id: string) => {
    if (!selectedId) return;
    setParticipantDataById((prev) => {
      const current = prev[selectedId];
      if (!current) return prev;
      return { ...prev, [selectedId]: { ...current, logs: current.logs.filter((l) => l.ID !== id) } };
    });
  }, [selectedId]);
  ...
  const handleSaved = useCallback((updated: LogItem[]) => {
    ...
    setParticipantDataById((prev) => {
      const current = prev[selectedId];
      if (!current) return prev;
      const logs = current.logs.map((l) => {
        const u = updated.find((u) => u.ID === l.ID);
        return u ? { ...l, ...u } : l;
      });
      const newItems = updated.filter((u) => !current.logs.some((l) => l.ID === u.ID));
      return { ...prev, [selectedId]: { ...current, logs: [...logs, ...newItems] } };
    });
            ...
            <div className="min-w-0 space-y-4">
              {selectedId && (
                <>
                  ...
                  {/* No data */}
                  {!detailLoading && participantData && dates.length === 0 && (
