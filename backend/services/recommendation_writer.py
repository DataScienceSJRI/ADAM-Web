...
def write_recommendations(
    ...
    Day 1 of the plan is set to `start_date` (defaults to tomorrow), so
    continuation weeks generated by the auto-scheduler can start the day after
    the previous week's plan ends instead of always defaulting to "tomorrow".
    ...
    for _, row in weekly_menu.iterrows():
        ...
        meal_date = (start + timedelta(days=day_num - 1)).isoformat()
...
def write_final_summary(
    ...
    """
    Writes the formatted weekly menu summary to FinalSummary table. set to `start_date`
    (defaults to tomorrow) so this matches write_recommendations() for the
    same plan. Internal computation columns (_opt_prop, _carb_g, etc.) are
    excluded.
    ...
    for _, row in final_summary_df.iterrows():
        try:
            meal_date = (start + timedelta(days=int(row.get("Day", 1)) - 1)).isoformat()
        except (TypeError, ValueError):
            meal_date = None
