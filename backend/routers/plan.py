...
def _run_plan_background(
    ...
    try:
        ...
        logger.info("Writing %d rows to recommendations for user_id=%s | unique days: %s", len(weekly_menu), user_id, unique_days)
...
def generate_plan(
    ...
    # The dashboard's "Retry" button re-runs the whole onboarding flow, which never sends
    # week_no or start_date -- so without this, retrying a failed LATER week (week 2+,
    # generated by the auto-scheduler in run_auto_next_week_job) would silently regenerate
    # week 1 starting tomorrow, instead of the actually-missing week continuing right after
    # the last real plan. Only kicks in when the caller genuinely omitted week_no
    # (model_fields_set), never overriding an explicit value -- the auto-scheduler always
    # passes week_no explicitly, so it's unaffected.
    if "week_no" not in body.model_fields_set:
        existing_plan = get_plan_status(user_id)
        if existing_plan["has_plan"]:
            latest_plan = existing_plan["plans"][0]  # get_plan_status sorts by start_date desc
            max_week = latest_plan["week_no"] or 0
            body.week_no = max_week + 1
            if latest_plan["end_date"]:
                body.start_date = (date.fromisoformat(latest_plan["end_date"]) + timedelta(days=1)).isoformat()
            logger.info(
                "user_id=%s: week_no omitted from request, inferred week_no=%d start_date=%s "
                "from most recent existing plan (week_no=%s, end_date=%s)",
                user_id, body.week_no, body.start_date, latest_plan["week_no"], latest_plan["end_date"],
            )
